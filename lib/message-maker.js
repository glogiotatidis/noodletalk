...
var getMessageType = exports.getMessageType = function(req){
  var meMatch = /^(\/me\s)/i;
  var pubKeyMatch = /^(\pubkey exchange\s)/i;
  ...
  if(req.body){
    ...
    if(nick.length > 0){
      ...
      return "activity";
    } else if (messageContent.match(pubKeyMatch)) {
      return "pubkey-exchange";
    }
  }
  return "";
}
...
exports.getMessage = function(noodle, channel, req, io, userList, messageType) {
  ...
  if(req.body) {
    ...
    if (messageType === "activity") {
      ...
      var _pubKey = "";
      if (pubkey) {
        _pubKey = 'key-data="' + pubkey  + '"';
      }
      message = '<em' + _pubKey  + '>' + req.session.nickname[channel] + ' ' + message.replace(meMatch, '') + '</em>';
      isAction = true;
